...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        int measure_unit_flag = 0;
        double total_measurement = 0;
        int measure_mode = 0;
        bool isMouseDown = false;
        bool first_measurementpt = false;
        UdpClient lght_udpClient = new UdpClient(9600);
       // string LGHT_ADDRESS = File.ReadAllText("LGHT_ADDRESS.txt");
        /// <summary>
        ...
        public FlightData()
        {
            ...
            */
            ...
            gMapControl1.Overlays.Add(MeasurePts);
        ...
        private void gMapControl1_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && isMouseDown == true)
            {
                ...
                if (first_measurementpt == false)
                {
                    ...
                    total_measurement = total_measurement + gMapControl1.MapProvider.Projection.GetDistance(start_point, new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng)) * 1000;
                    Marker_Measure.ToolTipText = "Dist: " + ((gMapControl1.MapProvider.Projection.GetDistance(start_point, new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng)) * 1000)).ToString("0") + "\nBearing: " + ((gMapControl1.MapProvider.Projection.GetBearing(start_point, new PointLatLng(MouseDownStart.Lat, MouseDownStart.Lng)))).ToString("0");
        ...
        private void send_light_cmd(int updown, int leftright)
        {
            try
            {
                if (lght_success == 0)
                {
                 //  lght_udpClient.Connect(LGHT_ADDRESS, 9600);
                    lght_success = 1;
                }
                else
                {
                    int up_pwm, right_pwm;
                    up_pwm = updown * 8 + 1100;
                    right_pwm = leftright * 8 + 1100;

                    string cmd_string = "$start," + up_pwm + "," + right_pwm;
                    Byte[] cmd_msg = Encoding.ASCII.GetBytes(cmd_string);
                    lght_udpClient.Send(cmd_msg, cmd_msg.Length);
                    MouseLabel.Text = cmd_string;
        ...
        private void restartroutebutton_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;
        ...
        private void back_2_center()
        {
            pictureBox1.Image = null;
            pictureBox1.Image = MissionPlanner.Properties.Resources.BlackBKG;
            Image i = pictureBox1.Image;
            using (Graphics g = Graphics.FromImage(i))
            {
                g.FillEllipse(Brushes.Red, new Rectangle(pictureBox1.Width / 2 - 15, (pictureBox1.Height * 5) / 10 - 15, 30, 30));
            }
            pictureBox1.Invalidate();
            send_light_cmd((int)(50), (int)(50));
        }
        private void LightCTRLBtn_Click(object sender, EventArgs e)
        {
        ...
        private void mouse_joy_func()
        {
        }
        bool JoystickisMouseDown = false;
        Thread mouse_joy_thread;
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            MouseLabel.Text = "Clicked";
            JoystickisMouseDown = true;
            mouse_joy_thread = new Thread(mouse_joy_func);
            mouse_joy_thread.Name = "Mouse Joystick Loop";
            mouse_joy_thread.IsBackground = true;
            mouse_joy_thread.Start();
        }

        private void pictureBox1_MouseLeave(object sender, EventArgs e)
        {
            MouseLabel.Text = "Left";
            JoystickisMouseDown = false;
            back_2_center();
        }
        long time_current_joy;
        long last_time_joy = -1;
        private void pictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            if (JoystickisMouseDown == true)
            {
                DateTime now = DateTime.Now;
                time_current_joy = now.Ticks;
                if (last_time_joy == -1)
                {
                    last_time_joy = time_current_joy;
                }
                int mousex = e.X;
                int mousey = e.Y;
                ...
                using (Graphics g = Graphics.FromImage(i))
                {
                    g.FillEllipse(Brushes.Red, new Rectangle(mousex - 15, mousey - 15, 30, 30));
                }
                if (mousex < 0)
                {
                    mousex = 0;
                ...
                if (mousex > pictureBox1.Width)
                {
                    mousex = pictureBox1.Width;
                }
                if (mousey > pictureBox1.Height)
                {
                    mousey = pictureBox1.Height;
                }
                float percent_x = (float)(mousex) / (float)(pictureBox1.Width) * (float)(100.0);
                float percent_y = (float)(mousey) / (float)(pictureBox1.Height) * (float)(100.0);
                pictureBox1.Invalidate();
                long delta_t = ((time_current_joy - last_time_joy));

                /*float fastest_rate = 3;
                percent_y = (percent_y - (float)(50.0)) * (float)(2.0);
                float increment_speed = (float)(delta_t)/(float)(1000.0) / fastest_rate * percent_y;*/
                //speed_percent_cmd_mouse_joy = speed_percent_cmd_mouse_joy + increment_speed;
                //if (speed_percent_cmd_mouse_joy > 100) { speed_percent_cmd_mouse_joy = 100; }
                //if (speed_percent_cmd_mouse_joy < 0) { speed_percent_cmd_mouse_joy = 0; }

                //MouseLabel.Text =" "+ delta_t;
                if (delta_t > (10000 * 100))
                {
                    //MouseLabel.Text = "Steering " + percent_x + " speed " + percent_y;
                    send_light_cmd((int)(percent_y), (int)(percent_x));
                    last_time_joy = time_current_joy;
        ...
        private void joystickSetup1_Load(object sender, EventArgs e)
        {
